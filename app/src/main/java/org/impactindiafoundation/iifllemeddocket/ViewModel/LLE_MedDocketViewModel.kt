...
class LLE_MedDocketViewModel(val LLE_MedDocketRespository: LLE_MedDocketRespository, val app:Application):ViewModel() {
    ...
    fun insertEyePreOPInvestigation(
        progressDialog: ProgressDialog,
        addEyePreOpInvestigationsRequest: AddEyePreOpInvestigationsRequest
    ...
    fun insertRefractiveError(
        progressDialog: ProgressDialog,
        data: AddRefractiveErrorRequest
    ) =viewModelScope.launch{
        try {
            if (ConstantsApp.checkInternetConenction(app))
            {

                getRefractiveErrorResponse.postValue(ResourceApp.Loading())
                val data=LLE_MedDocketRespository.insertRefractiveError(data)
                getRefractiveErrorResponse.postValue(handleInsertRefractiveError(data)!!)
    ...
    private fun handleInsertRefractiveError(response: Response<getRefractiveErrorResponse>): ResourceApp<getRefractiveErrorResponse>? {
    ...
    fun addPrecriptionGlassesResponse(
        progressDialog: ProgressDialog,
        AddPrecriptionFinal: AddPrecriptionFinal,
