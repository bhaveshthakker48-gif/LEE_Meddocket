...
class NewVitalsActivity : BaseActivity() {
    ...
    private fun initObserver() {
        ...
        patientReportVM.insertPatientReportResponse.observe(this) {
            when (it.status) {
                ...
                Status.SUCCESS -> {
                    try {
                        Utility.successToast(this@NewVitalsActivity, "Form Submitted Successfully")
                        onBackPressed()
                    } catch (e: Exception) {
                        Log.e("FormSaveError", e.message!!)
                    }
    ...
    private fun saveVitalsForm() {
        ...
        val pulseRate =
            if (binding.etPulseRate.text.isNullOrEmpty()) 0 else binding.etPulseRate.text.toString()
                .toInt()
